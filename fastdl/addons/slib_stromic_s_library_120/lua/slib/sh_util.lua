...
if CLIENT then
    ...
    file.CreateDir("slib/image")

    local encodeFromURL = function(url)
        local encoded = util.Base64Encode(url)
    ...
    local getStoredImage = function (url)
        local encodedImgName = encodeFromURL(url)

        if !matCache[encodedImgName] then
            if file.Exists("slib/image/"..encodedImgName..".png", "DATA") then
                matCache[encodedImgName] = Material("data/slib/image/"..encodedImgName..".png", "noclamp smooth")

                if matCache[encodedImgName]:IsError() then
                    file.Delete("slib/image/"..encodedImgName..".png")

                    matCache[encodedImgName] = nil
                end
            end
        end

        return matCache[encodedImgName]
    end

    slib.ImgurGetMaterial = function(imgurId) -- RETURNS THE LOADING MATERIAL UNTIL IMAGE IS FOUND!!!
        return slib.getImageMaterialFromURL("https://i.imgur.com/"..imgurId..".png")
    end

    slib.getImageMaterialFromURL = function(url)
        local storedMaterial = getStoredImage(url)

        if storedMaterial then
            return storedMaterial
        else
            slib.downloadImageFromURL(url)
        end

        return loading_ico, true
    end

    slib.downloadImageFromURL = function(url)
        if fetching[url] then return end
        ...
        http.Fetch(url,
            function(body)
                file.Write("slib/image/"..encodedImgName..".png", body)
                matCache[encodedImgName] = Material("data/slib/image/"..encodedImgName..".png", "noclamp smooth")
